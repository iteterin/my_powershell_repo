...
process {
    foreach ($p in Resolve-Path -Path $Path | Select-Object -ExpandProperty Path -ErrorAction SilentlyContinue) {
        ...
        $result_fin = New-Object PSObject -Property @{
            Path = $p
...
$drives | %{
    $drive = $_
    $dir = Get-ChildItem -Path $drive -Directory
    $dir | %{
        $data = ""
        $current_dir = $_.FullName
        Write-Host -ForegroundColor Yellow $current_dir
        $data = Get-FolderSize $current_dir -Verbose
        $ht.$current_dir = $data[0].TotalBytes
        $data[0].Path; $data[0].TotalBytes
    }
